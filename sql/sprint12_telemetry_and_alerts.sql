-- Sprint 12: Telemetry and Advanced Alerts
...
-- Allow anonymous inserts (anyone can send telemetry)
DROP POLICY IF EXISTS "Allow anonymous inserts" ON public.visitor_analytics;
CREATE POLICY "Allow anonymous inserts" ON public.visitor_analytics FOR INSERT WITH CHECK (true);

-- Allow only admins to read telemetry
DROP POLICY IF EXISTS "Admins can view telemetry" ON public.visitor_analytics;
CREATE POLICY "Admins can view telemetry" ON public.visitor_analytics FOR SELECT USING (public.is_admin(auth.jwt() ->> 'email'));

-- 2. Update submit_vote to notify ALL admins when 100% turnout is reached
...
BEGIN
    ...
    IF v_expected_voters IS NOT NULL AND v_expected_voters > 0 THEN
        ...
        IF v_total_votes = v_expected_voters THEN
            -- Dynamically insert notification for all active admins (DISTINCT ensures no duplicates)
            INSERT INTO public.email_notifications (notification_type, recipient_email)
            SELECT DISTINCT 'ALL_VOTED_ADMIN_ALERT', email 
            FROM public.admins 
            WHERE active = true;
        END IF;
    END IF;
...
-- 3. Create send_closing_reminders RPC
CREATE OR REPLACE FUNCTION public.send_closing_reminders()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
    ...
    -- Insert CLOSING_WARNING for all pending voters (DISTINCT guarantees 1 per person)
    INSERT INTO public.email_notifications (notification_type, recipient_email)
    SELECT DISTINCT 'CLOSING_WARNING', email 
    FROM public.eligible_students 
    WHERE status = 'pending';

    -- Insert ADMIN_PENDING_LIST for the admin who triggered it
    INSERT INTO public.email_notifications (notification_type, recipient_email)
    VALUES ('ADMIN_PENDING_LIST', admin_email);

    -- Log action
    INSERT INTO public.vote_activity_log (actor, event_type)
    VALUES (admin_email, 'SENT_CLOSING_REMINDERS');
END;
$$;

GRANT EXECUTE ON FUNCTION public.send_closing_reminders() TO authenticated;
